...
/**
 * @brief Records the position of a non-escaped wildcard in a token.
 *
 * Grows the dynamic array of wildcard indices, keeping it terminated by -1.
 *
 * @param i Index of the wildcard in the token string.
 * @param wcs Pointer to the wildcard index array pointer.
 *
 ...
 * If no match is found, returns a single copy of the original token node.
 * 
 * @param t Token node.
 * 
