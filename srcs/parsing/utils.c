...
/**
 * @brief Checks if a character is any parsing operator or bracket.
 *
 * @param c Character to check.
 *
 * @return TRUE if the character is an operator/bracket, FALSE otherwise.
 */
...
/**
 * @brief Reports a syntax error at the given index in the current command.
 *
 * Prints an error message, underlines the offending position, and sets the
 * global exit code.
 *
 * @param data Parser state container.
 * @param i Index in the command string where the error occurred.
 *
 ...
 * only the '*' wildcard. Escaped wildcards (i.e. literal asterisks) are handled
